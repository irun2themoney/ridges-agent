...
def _transpose_minimal() -> str:
    return (
        ...
        "    # pad right with spaces to maxlen\n"
        "    padded = [r.ljust(maxlen) for r in rows]\n"
        "    # transpose: collect column by column\n"
        "    cols = []\n"
        "    for c in range(maxlen):\n"
        "        col = ''.join(padded[r][c] for r in range(len(padded)))\n"
...
def _two_bucket_minimal() -> str:
    return (
        ...
        "    # BFS to find shortest path, starting with specified bucket filled\n"
        "    if start_bucket == 'one':\n        initial = (bucket_one, 0, 1)\n    else:\n        initial = (0, bucket_two, 1)\n"
        "    q = deque([initial])\n"
        "    seen = set()\n"
        "    while q:\n"
        "        a, b, moves = q.popleft()\n"
        ...
        "        next_states = [\n"
        "            (bucket_one, b, moves+1),  # fill one\n"
        "            (a, bucket_two, moves+1),  # fill two\n"
        "            (0, b, moves+1),           # empty one\n"
        "            (a, 0, moves+1),           # empty two\n"
        "            (max(0, a - (bucket_two - b)), min(bucket_two, b + a), moves+1),  # one to two\n"
        "            (min(bucket_one, a + b), max(0, b - (bucket_one - a)), moves+1),    # two to one\n"
        "        ]\n"
        "        for na, nb, nm in next_states:\n"
