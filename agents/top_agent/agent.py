...
class FixTaskEnhancedToolManager(EnhancedToolManager):
    ...
    def get_approval_for_solution(self, solutions: list[str], selected_solution: int, reason_for_selection: str) -> str:
        ...
        for solution in solutions:
            # Support both "Solution 1:" and "Solution 1 -" formats
            sols = re.split(r"(Solution \d+[\s\-:]+)", solution)
            sols = [f"{sols[i]}{sols[i + 1]}" for i in range(1, len(sols), 2) if i + 1 < len(sols)]  # Combine the split parts correctly
            parsed_solutions.extend(sols)

        # If parsing produced results, use them; otherwise keep original solutions
        if parsed_solutions:
